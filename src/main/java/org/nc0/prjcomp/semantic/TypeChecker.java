...
public class TypeChecker extends BaseVisitor<Type> {
    ...
    public Type visit(Id i) {
        ...
        if (t == null) {
            errors.add(i, "Impossible de trouver le type de la variable " + i.getName());
            return errType;
        }
        return t;
    ...
    public Type visit(MethodDecl md) {
        ...
        if (typeRetour == null) {
            ...
            errors.add(md, "Type de retour attendu : " + ret + ". Trouvé : " + typeRetour.toString());
        }
    ...
    public Type visit(StatAff sa) {
        ...
        if (!t1.equals(t2)) {
            errors.add(sa, "Impossible d’affecter l’expression " + e + " de type " + t2 + " à la variable " + sa.getId().getName() + " de type " + t1);
        }
        return statType;
    }
    ...
    public Type visit(StatIf s) {
        ...
        if (ret1 != null && ret2 != null && !ret1.equals(ret2)) {
            errors.add(s, "Les types de valeurs de retour diffèrent. Trouvé " + ret1 + " (branche \"then\") et " + ret2 + " (branche \"else\").");
            typeRetour = null;
        }
    ...
    public Type visit(StatPrint sp) {
        ...
        if (t.toString() != "int") {
            errors.add(sp, "L’Impression ne supporte que les entiers. (trouvé " + t + ")");
        }
        return statType;
    }
    ...
    public Type visit(StatWhile s) {
        ...
        if (t.toString() != "bool") {
            errors.add(s, "La condition du while n’est pas du type bool. (trouvé " + t + ")");
        }
    ...
    public Type visit(ExpBin e) {
        ...
        if (!tg.equals(td)) {
            errors.add(e, "Les expressions doivent être de même type" + " pour l’opération " + op.toString() + " (trouvé : " + tg + " et " + td + ").");
            return errType;
        }
        Type t = null;
        switch (op) {
            case ADD, MIN, MULT, DIV:
                if (tg.toString().equals("int")) {
                    t = new TypePrim(e.getPosition(), TypePrim.Prim.INT);
                } else {
                    errors.add(e, "expressions de type int" + " attendues pour l’opération " + op + " (trouvé : " + tg + ").");
                    t = errType;
                }
                break;
            case AND, OR:
                if (tg.toString().equals("bool")) {
                    ...
                    errors.add(e, "expressions de type bool" + " attendues pour l’opération " + op + " (trouvé : " + tg + ")."
    ...
    public Type visit(ExpCallMethod ec) {
        ...
        if (sig == null) {
            errors.add(ec, "Méthode " + id + " non trouvée");
            return errType;
        }
        List<Formal> lf = sig.params();
        Type tReturn = sig.returnType();
        if (lf.size() != le.size()) {
            errors.add(ec, "La méthode " + id + " doit recevoir " + lf.size() + " arguments. (trouvés : " + le.size() + ")");
            return errType;
        }
        Type tParam = null;
        Type tExp = null;
        for (int i = 0; i < lf.size(); i++) {
            ...
            if (!tParam.equals(tExp)) {
                errors.add(ec, "Argument de " + id + " en position " + (i + 1) + " : attendu exp. de type " + tParam + " (trouvé : " + tExp + ")");
                return errType;
            }
        }
        return tReturn;
    ...
    public Type visit(ExpId e) {
        ...
        if (t == null) {
            errors.add(e, "Impossible de trouver le type de la variable " + e.getValue());
            return errType;
        }
